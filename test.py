...
def visualize(embedding,labels):
    ...
    df["D-1"] = z[:, 0]
    df["D-2"] = z[:, 1]
    # 绘制散点图   如果没有逗号，就会说list没有get_figure()，可见sns.scatterplot的返回值是一个list，但有点奇怪
    fig, = sns.scatterplot(x="D-1", y="D-2", hue=df.y.tolist(),
                           palette=sns.color_palette("hls", labels.max().item() + 1),
                           data=df).set(title=" ")
    plt.show()
    # 创建路径
    path_img = "./img"
    path = os.path.join(path_img, 'imdb')
    os.makedirs(path, exist_ok=True)
    # 确定文件名
    cur_time = time.strftime('%Y%m%d_%H%M%S', time.localtime())
    fig_path = './img/{}/pic-{}.png'.format('imdb', cur_time)
...
def main():
    ...
    visualize(train_features,labels)
